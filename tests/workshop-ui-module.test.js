...
test('Platform specs are the Review and Package source of truth', async () => {
  ...
  const generic = specs.getPlatformSpec(specs.PlatformSpecKeys.GENERIC_PNG_PACK);
  ...
  assert.equal(generic.key, specs.PlatformSpecKeys.GENERIC_PNG_PACK);
  assert.equal(generic.deliveryLabel, '通用 PNG 貼圖包');
  assert.equal(generic.status, specs.PlatformSpecStatuses.PLANNED);
  assert.equal(generic.supportsMain, false);
  assert.equal(generic.supportsTab, false);
  ...
  assert.equal(specs.getPlatformNaming(generic.key).stickerFileNameAt(0), '01.png');
