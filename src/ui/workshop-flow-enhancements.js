...
const ENHANCEMENT_STYLE_ID = 'stixio-workflow-enhancement-style';
...
export function enhanceWorkflowExperience(root = document.getElementById('app')) {
  if (!root || root.dataset.workflowEnhancements === 'ready') return;
  root.dataset.workflowEnhancements = 'ready';
  ...
  const scheduleApply = () => {
    ...
    queueMicrotask(() => {
      scheduled = false;
      applyEnhancements(root, collapsed);
    });
  };
...
function applyEnhancements(root, collapsed) {
  ...
  applyEmptyState(root, workspace, stageId, metrics);
...
function ensureCollapseControls(workspace) {
  ...
  let controls = overview.querySelector('[data-workflow-collapse-controls]');
  if (!controls) {
...
function applyCollapsedColumns(workspace, collapsed) {
  const leftColumn = workspace.querySelector('[data-workflow-column="left"]');
  const rightColumn = workspace.querySelector('[data-workflow-column="right"]');
  const leftAvailable = Boolean(leftColumn && leftColumn.childElementCount && !leftColumn.dataset.emptyHidden);
  const rightAvailable = Boolean(rightColumn && rightColumn.childElementCount && !rightColumn.dataset.emptyHidden);

  workspace.dataset.collapseLeft = String(leftAvailable && collapsed.left);
  workspace.dataset.collapseRight = String(rightAvailable && collapsed.right);

  if (leftColumn) leftColumn.dataset.collapsed = String(leftAvailable && collapsed.left);
  if (rightColumn) rightColumn.dataset.collapsed = String(rightAvailable && collapsed.right);

  updateCollapseButton(workspace, 'left', leftAvailable, collapsed.left);
  updateCollapseButton(workspace, 'right', rightAvailable, collapsed.right);
}

function updateCollapseButton(workspace, side, available, collapsed) {
  ...
  const sideLabel = side === 'left' ? '左欄' : '右欄';
  button.textContent = collapsed ? `顯示${sideLabel}` : `收合${sideLabel}`;
}

function applyEmptyState(root, workspace, stageId, metrics) {
  restoreEmptyHiddenElements(workspace);
  workspace.querySelector('[data-workflow-empty]')?.remove();

  const emptyState = getWorkflowEmptyState(stageId, metrics);
  ...
  guide.innerHTML = `
    <div class="max-w-md">
      ...
      <p class="mt-6 text-[10px] font-black uppercase tracking-[.24em] text-slate-400">${emptyState.eyebrow}</p>
      <h3 class="mt-2 text-2xl font-black text-slate-950">${emptyState.title}</h3>
      <p class="mt-3 text-sm font-bold leading-6 text-slate-500">${emptyState.description}</p>
      <button type="button" data-workflow-empty-action="${emptyState.action}" class="mt-6 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white">${emptyState.actionLabel}</button>
    </div>`;
  center.appendChild(guide);
}

function restoreEmptyHiddenElements(workspace) {
...
function collectMetrics(root) {
  ...
  const reviewCardCount = root.querySelectorAll('[data-review-card="true"]').length;
  return {
    sourceCount: sourceRows.length,
    frameCount: Math.max(frameCountFromRows, reviewCardCount)
  };
}
...
function installEnhancementStyle() {
  ...
  style.textContent = `
    ...
    @media (max-width: 767px) {
      ...
      .mobile-workflow-button[data-active="true"] {
        background: #0f172a;
        color: #ffffff;
      }
