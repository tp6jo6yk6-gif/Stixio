...
const DEFAULT_SETTINGS = applyStickerPreset({
  ...
  reviewSearch: '',
  reviewPackCriterion: 'line-standard',
...
function renderDetectionPanel() {
  ...
  return `<section class="rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><p class="text-[10px] font-black uppercase tracking-[.2em] text-emerald-600">Layout · Detection Engine</p><h2 class="mt-1 text-xl font-black">切割與智能貼合</h2><p class="mt-2 text-xs font-bold text-slate-400">導入圖片會先用智能判定；只有判定不準時再切換手動。</p><div class="mt-4 grid grid-cols-3 gap-2">${layoutButton('auto','智能')}${layoutButton('1x1','單圖')}${layoutButton('2x2','2×2')}${layoutButton('3x3','3×3')}${layoutButton('custom','自訂')}</div><button id="manualLayoutToggle" class="mt-3 w-full rounded-2xl bg-slate-100 py-3 text-xs font-black text-slate-700" type="button" aria-expanded="${manualOpen?'true':'false'}">${manualOpen?'收起手動設定':'智能判定不準？展開手動設定'}</button><div id="manualLayoutFields" class="mt-4 grid grid-cols-2 gap-3 ${manualOpen?'':'hidden'}">${numberInput('rowsInput','Rows',state.settings.rows,1,12)}${numberInput('colsInput','Cols',state.settings.cols,1,12)}${numberInput('marginXInput','Margin X',state.settings.marginX,0,500)}${numberInput('marginYInput','Margin Y',state.settings.marginY,0,500)}${numberInput('gapXInput','Gap X',state.settings.gapX,0,500)}${numberInput('gapYInput','Gap Y',state.settings.gapY,0,500)}</div><label class="mt-4 flex items-center justify-between rounded-2xl bg-slate-50 p-3 text-sm font-black"><span>拖曳後智能貼合內容</span><input id="smartSnapInput" type="checkbox" ${state.settings.smartSnap?'checked':''}></label><button id="detectBtn" class="mt-4 w-full rounded-2xl bg-slate-950 py-3 text-sm font-black text-white">重新偵測目前原圖</button></section>`;
}
...
function reviewPackRuleSlots(){
  const stickerCount=[8,16,24,32,40].includes(Number(state.settings.reviewStickerCount))?Number(state.settings.reviewStickerCount):8;
  return {stickerCount,main:state.settings.reviewIncludeMain!==false,tab:state.settings.reviewIncludeTab!==false,total:stickerCount+(state.settings.reviewIncludeMain!==false?1:0)+(state.settings.reviewIncludeTab!==false?1:0)};
}
function syncReviewPackRuleControls(){
  ...
  if(criterion)criterion.value=state.settings.reviewPackCriterion||'line-standard';
  if(count)count.value=String(reviewPackRuleSlots().stickerCount);
  if(main)main.checked=state.settings.reviewIncludeMain!==false;
  if(tab)tab.checked=state.settings.reviewIncludeTab!==false;
  if(summary){const slots=reviewPackRuleSlots();summary.textContent=`將選出 ${slots.total} 張：${slots.main?'main + ':''}${slots.tab?'tab + ':''}${slots.stickerCount} 張貼圖；其餘放入備選。`;}
}
function applyReviewPackRule(){
  const slots=reviewPackRuleSlots();
  state.settings.reviewPackCriterion='line-standard';
  ...
  state.destinationController?.importState?.({...destinationState,activeKey:DestinationProfileKeys.STANDARD,activeRole:AssetRoles.STICKER});
  state.settings=applyDestinationProfileToSettings(state.settings,getBuiltInDestinationProfile(DestinationProfileKeys.STANDARD),AssetRoles.STICKER);
  setFrames(applyLineStickerReviewRule(frames(),{stickerCount:slots.stickerCount,includeMain:slots.main,includeTab:slots.tab}));
...
function renderReviewGrid(){
  ...
  virtual.items.forEach(item=>{const frame=item.frame,canvas=renderFrame(frame),card=document.createElement('article');card.draggable=canReorder;card.dataset.frameId=frame.id;card.dataset.sourceId=frame.sourceImageId;card.dataset.reviewCard='true';card.dataset.reviewSeverity=item.highestSeverity||'clean';card.dataset.reviewApproved=String(item.approved);card.dataset.exportSelected=String(item.exportSelected);const severityClass=item.hasErrors?'border-rose-500 bg-rose-50':item.hasWarnings?'border-amber-400 bg-amber-50':item.approved?'border-emerald-400 bg-emerald-50':frame.id===state.selectedFrameId?'border-sky-400 bg-sky-50':'border-slate-200 bg-white';card.className=`flex min-h-[252px] flex-col rounded-3xl border p-2 transition ${severityClass}`;const badge=item.hasErrors?'錯誤':item.hasWarnings?'警告':item.approved?'已核准':'待核准';card.innerHTML=`<button class="preview relative block aspect-[370/320] w-full overflow-hidden rounded-2xl"><img class="h-full w-full object-contain" alt=""><span class="absolute left-2 top-2 rounded-full bg-slate-950/80 px-2 py-1 text-[10px] font-black text-white">${badge}</span><label class="absolute right-2 top-2 flex items-center rounded-full bg-white/90 px-2 py-1 text-[10px] font-black shadow-sm"><input class="export-check mr-1" type="checkbox" ${item.exportSelected?'checked':''}>匯出</label></button><div class="mt-2 flex items-center justify-between gap-2"><span class="text-xs font-black">#${item.index+1} · ${item.kb}KB</span><span class="rounded-full ${item.approved?'bg-emerald-100 text-emerald-700':item.hasErrors?'bg-rose-100 text-rose-700':item.hasWarnings?'bg-amber-100 text-amber-700':'bg-slate-100 text-slate-500'} px-2 py-1 text-[10px] font-black">${badge}</span></div><div class="mt-1 truncate text-[10px] text-slate-400" title="${escapeHtml(item.sourceName)}">${escapeHtml(item.sourceName)}</div><div class="mt-auto grid min-h-[78px] gap-2 pt-2"><div class="truncate text-[10px] text-slate-400" title="${escapeHtml(item.fileName||'備選')}">${escapeHtml(item.fileName||'備選')}</div><div class="flex gap-2"><button class="review-approve flex-1 rounded-xl ${item.approved?'bg-emerald-500 text-white':'bg-slate-100'} py-2 text-xs font-black">${item.approved?'撤銷核准':'核准'}</button><button class="single-download rounded-xl bg-slate-950 px-3 py-2 text-xs font-black text-white">儲存 PNG</button></div><select class="role-select w-full rounded-xl bg-slate-100 px-2 py-1 text-xs font-black" title="整包角色可在 Package 編排">${roles.map(role=>`<option value="${role}" ${packageRole(frame)===role?'selected':''}>${roleLabel(role)}</option>`).join('')}</select></div>`;
...
function refreshReviewControls(){const filter=document.getElementById('reviewFilterInput'),sort=document.getElementById('reviewSortInput');if(filter)filter.value=state.settings.reviewFilter;if(sort)sort.value=state.settings.reviewSort;syncReviewPackRuleControls();document.querySelectorAll('.review-bg').forEach(button=>button.classList.toggle('ring-2',button.dataset.reviewBg===state.settings.reviewBackground));const progress=getReviewProgress(allReviewItems()),allClear=progress.selected>0&&progress.approved===progress.selected&&progress.withErrors===0,approveButton=document.getElementById('reviewApproveCleanBtn'),nextHint=document.getElementById('reviewNextStepHint');if(approveButton){approveButton.disabled=allClear;approveButton.classList.toggle('opacity-40',allClear);approveButton.classList.toggle('bg-slate-100',allClear);approveButton.classList.toggle('bg-emerald-300',!allClear);approveButton.title=allClear?'全部已核准，可進入 Package':'';}if(nextHint)nextHint.classList.toggle('hidden',!allClear);const exportButton=document.getElementById('exportZipBtn');if(exportButton){exportButton.disabled=!state.reviewReport?.canPackage;exportButton.classList.toggle('opacity-40',exportButton.disabled);exportButton.title=exportButton.disabled?'Review 尚未核准或仍有錯誤':'';}}
