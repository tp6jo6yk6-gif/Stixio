...
function bindStaticEvents(root) {
  ...
  root.querySelector('#reviewPackCriterionInput')?.addEventListener('change',event=>{state.settings.reviewPackCriterion=event.target.value;syncReviewPackRuleControls();});
  root.querySelector('#reviewStickerCountInput')?.addEventListener('change',event=>{state.settings.reviewStickerCount=Number(event.target.value)||8;syncReviewPackRuleControls();});
  root.querySelector('#reviewIncludeMainInput')?.addEventListener('change',event=>{state.settings.reviewIncludeMain=event.target.checked;syncReviewPackRuleControls();});
  root.querySelector('#reviewIncludeTabInput')?.addEventListener('change',event=>{state.settings.reviewIncludeTab=event.target.checked;syncReviewPackRuleControls();});
...
function syncReviewPackRuleControls(){
  const criterion=document.getElementById('reviewPackCriterionInput'),count=document.getElementById('reviewStickerCountInput'),main=document.getElementById('reviewIncludeMainInput'),tab=document.getElementById('reviewIncludeTabInput'),summary=document.getElementById('reviewPackRuleSummary');
