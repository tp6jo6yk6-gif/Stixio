import {
  ...
  getAvailablePlatformSpecs,
  getPlatformSpec,
  normalizePlatformStickerCount,
  applyPlatformStickerReviewRule,
  nextReviewFrameId,
  ReviewFilterModes,
  ReviewSortModes,
  ReviewIssueSeverity,
...
const DEFAULT_SETTINGS = applyStickerPreset({
  ...
  reviewSearch: '',
  reviewPackCriterion: PlatformSpecKeys.MESSAGING_STANDARD,
  reviewStickerCount: 8,
  reviewIncludeMain: true,
  reviewIncludeTab: true,
  reviewZoom: 1,
  refineZoom: 1,
  maskTool: 'view',
  maskSize: 15,
    ...
    <div class="min-h-screen bg-[#f6f3ec] text-slate-950">
      <header class="sticky top-0 z-40 border-b border-slate-900/10 bg-[#f6f3ec]/95 backdrop-blur-xl">
        ...
        <nav aria-label="Workshop workflow" class="mx-auto grid max-w-[1600px] grid-cols-2 gap-2 px-5 pb-4 md:grid-cols-4">
          ${stageLink('layout','Layout','匯入與裁切','bg-slate-950 text-white','text-emerald-300')}
          ${stageLink('refine','Refine','修補與去背','border border-slate-900/10 bg-white','text-rose-500')}
          ${stageLink('review','Review','檢查與編排','border border-slate-900/10 bg-white','text-sky-600')}
          ${stageLink('package','Package','交付確認','border border-slate-900/10 bg-white','text-amber-600')}
        </nav>
      </header>
      <main class="mx-auto grid max-w-[1600px] grid-cols-1 gap-5 px-5 py-6 xl:grid-cols-[360px_1fr_340px]">
        ...
        <section class="space-y-4">${renderSourceEditor()}${renderRefineEditor()}${renderReviewLedBoard()}${renderDeliveryConfirmBoard()}</section>
        <aside class="space-y-4">${renderSourceListPanel()}${renderSelectedPanel()}${renderReviewDecisionPanel()}${renderDeliveryConfirmPanel()}</aside>
      </main>
    </div>`;
}
...
function renderImportPanel() {
  return `<section id="stage-layout" class="scroll-mt-40 rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><p class="text-[10px] font-black uppercase tracking-[.2em] text-emerald-600">Layout · Artwork Engine</p><h2 class="mt-1 text-xl font-black">匯入圖片</h2><p class="mt-2 text-sm text-slate-500">把圖片拖進來，Stixio 會先做智能判定；需要時再展開手動設定。</p><label id="dropZone" class="mt-4 block cursor-pointer rounded-3xl border-2 border-dashed border-slate-300 bg-slate-50 p-6 text-center hover:border-emerald-400"><input id="fileInput" type="file" accept="image/*" multiple class="hidden"><div class="text-3xl font-black text-emerald-500">＋</div><div class="mt-2 font-black">拖曳圖片或點擊匯入</div><div class="text-xs text-slate-400">支援多張圖片，先自動偵測再微調。</div></label></section>`;
}

function renderDetectionPanel() {
  const manualOpen=Boolean(state.settings.manualLayoutOpen);
  return `<section class="rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><p class="text-[10px] font-black uppercase tracking-[.2em] text-emerald-600">Layout · Detection Engine</p><h2 class="mt-1 text-xl font-black">切割與智能貼合</h2><p class="mt-2 text-xs font-bold text-slate-400">預設先智能判定；只有判定錯誤時再展開手動列數、間距與邊界。</p><div class="mt-4 grid grid-cols-3 gap-2">${layoutButton('auto','智能')}${layoutButton('1x1','單張')}${layoutButton('2x2','2×2')}${layoutButton('3x3','3×3')}${layoutButton('custom','自訂')}</div><button id="manualLayoutToggle" class="mt-3 w-full rounded-2xl bg-slate-100 py-3 text-xs font-black text-slate-700" type="button" aria-expanded="${manualOpen?'true':'false'}">${manualOpen?'收合手動設定':'Margin / Gap 需要時再展開'}</button><div id="manualLayoutFields" class="mt-4 grid grid-cols-2 gap-3 ${manualOpen?'':'hidden'}">${numberInput('rowsInput','Rows',state.settings.rows,1,12)}${numberInput('colsInput','Cols',state.settings.cols,1,12)}${numberInput('marginXInput','Margin X',state.settings.marginX,0,500)}${numberInput('marginYInput','Margin Y',state.settings.marginY,0,500)}${numberInput('gapXInput','Gap X',state.settings.gapX,0,500)}${numberInput('gapYInput','Gap Y',state.settings.gapY,0,500)}</div><label class="mt-4 flex items-center justify-between rounded-2xl bg-slate-50 p-3 text-sm font-black"><span>拖曳後智能貼合內容</span><input id="smartSnapInput" type="checkbox" ${state.settings.smartSnap?'checked':''}></label><button id="detectBtn" class="mt-4 w-full rounded-2xl bg-slate-950 py-3 text-sm font-black text-white">重新偵測目前原圖</button></section>`;
}
...
function renderOutputPanel() {
  ...
  return `<section id="package-rules-panel" class="rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><p class="text-[10px] font-black uppercase tracking-[.2em] text-amber-600">Package · Rules Engine</p><h2 class="mt-1 text-xl font-black">輸出規格</h2><div class="mt-4 grid grid-cols-2 gap-3"><div class="rounded-2xl bg-slate-50 p-3"><div class="text-[10px] font-black uppercase text-slate-400">Role</div><div id="outputRoleLabel" class="mt-1 text-sm font-black">${roleLabel(output.role)}</div></div><div class="rounded-2xl bg-slate-50 p-3"><div class="text-[10px] font-black uppercase text-slate-400">File limit</div><div id="outputFileLimitLabel" class="mt-1 text-sm font-black">${Math.ceil(output.maxFileSizeBytes/1024)}KB</div></div></div><div class="mt-3 grid grid-cols-2 gap-3">${numberInput('targetWInput','寬度',output.width,1,8192)}${numberInput('targetHInput','高度',output.height,1,8192)}</div>${rangeInput('safeMarginInput','安全邊距',output.safeMargin,0,safeMax)}<div class="mt-3"><div class="mb-2 text-xs font-black text-slate-500">對齊方式</div><div class="grid grid-cols-2 gap-2">${alignButton('center','置中')}${alignButton('bottom','貼齊底部')}</div></div><div id="outputRuleSummary" class="mt-3 rounded-2xl bg-emerald-50 p-3 text-sm font-black text-emerald-800">${output.width} × ${output.height}px · safe ${output.safeMargin}px</div><p class="mt-3 text-[10px] font-bold text-slate-400">這裡顯示目前平台規格的輸出限制；完整編排請在 Review 決定。</p></section>`;
}

function renderRefinePanel() {
  return `<section id="refine-settings-panel" class="rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><p class="text-[10px] font-black uppercase tracking-[.2em] text-rose-500">Refine · Processing Engine</p><h2 class="mt-1 text-xl font-black">去背與修補</h2><label class="mt-4 flex items-center justify-between rounded-2xl bg-slate-50 p-3 text-sm font-black"><span>啟用背景色去背</span><input id="chromaEnabledInput" type="checkbox" ${state.settings.chromaEnabled?'checked':''}></label><div class="mt-3 flex gap-2"><input id="chromaColorInput" type="color" value="${rgbToHex(state.settings.chromaColor)}" class="h-10 w-14 rounded-xl"><button id="pickerBtn" class="flex-1 rounded-xl bg-slate-100 text-xs font-black">選取背景色</button></div>${rangeInput('toleranceInput','容差',state.settings.tolerance,5,120)}<label class="mt-3 flex items-center justify-between rounded-2xl bg-slate-50 p-3 text-sm font-black"><span>只處理外部連通背景</span><input id="exteriorInput" type="checkbox" ${state.settings.exteriorOnly?'checked':''}></label><label class="mt-3 flex items-center justify-between rounded-2xl bg-slate-50 p-3 text-sm font-black"><span>自動清除雜點</span><input id="despeckleInput" type="checkbox" ${state.settings.autoDespeckle?'checked':''}></label>${rangeInput('despeckleSizeInput','最小雜點',state.settings.despeckleMinSize,1,500)}${rangeInput('shrinkInput','收縮半徑',state.settings.shrinkRadius,0,8)}${rangeInput('featherInput','羽化半徑',state.settings.featherRadius,0,8)}<label class="mt-3 flex items-center justify-between rounded-2xl bg-slate-50 p-3 text-sm font-black"><span>白邊</span><input id="borderInput" type="checkbox" ${state.settings.whiteBorderEnabled?'checked':''}></label><div class="mt-3 flex items-end gap-3"><label class="text-xs font-black text-slate-500">白邊顏色<input id="borderColorInput" type="color" value="${state.settings.borderColor}" class="mt-1 h-10 w-14 rounded-xl"></label><div class="flex-1">${rangeInput('borderSizeInput','白邊大小',state.settings.whiteBorderSize,0,25)}</div></div><div class="mt-4 grid grid-cols-2 gap-2"><button id="renderSelectedBtn" class="rounded-2xl bg-sky-50 py-3 text-xs font-black text-sky-800">渲染單張</button><button id="renderAllBtn" class="rounded-2xl bg-emerald-300 py-3 text-xs font-black text-slate-950">渲染全部</button></div><button id="resetRefineSettingsBtn" class="mt-2 w-full rounded-2xl bg-slate-100 py-3 text-xs font-black">重設 Refine 設定</button></section>`;
}
function renderSourceEditor() {
  return `<section class="rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><div class="mb-4 flex items-center justify-between"><div><p class="text-[10px] font-black uppercase tracking-[.2em] text-emerald-600">Layout 蝜?Frame Editor</p><h2 class="text-xl font-black">?賹?????綜筆????</h2></div><div id="sourceStatus" class="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-500">?垮謓??穿</div></div><div class="grid min-h-[460px] place-items-center overflow-auto rounded-3xl border border-slate-200 bg-slate-100 p-4"><canvas id="sourceCanvas" class="max-h-[70vh] max-w-full cursor-crosshair rounded-xl shadow-xl"></canvas></div></section>`;
}

function renderRefineEditor() {
  const frame = selectedFrame();
  if (!frame) {
    return `<section id="stage-refine" class="scroll-mt-40 rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><div class="flex flex-wrap items-center justify-between gap-3"><div><p class="text-[10px] font-black uppercase tracking-[.2em] text-rose-500">Refine · Manual Tools</p><h2 class="text-xl font-black">單張遮罩修補工作台</h2></div></div><div id="refineEmptyState" class="mt-4 grid min-h-[220px] place-items-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center"><div><div class="text-sm font-black text-slate-700">尚未選取 Frame</div><p class="mt-2 max-w-md text-xs font-bold text-slate-400">請先在 Layout 或 Review 選取一張圖片，再進行去背與遮罩修補。</p></div></div></section>`;
  }
  const mode = state.settings.refineViewMode;
  const showEditor = mode !== 'result';
  const showResult = mode !== 'mask';
  return `<section id="stage-refine" class="scroll-mt-40 rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><div class="flex flex-wrap items-center justify-between gap-3"><div><p class="text-[10px] font-black uppercase tracking-[.2em] text-rose-500">Refine · Manual Tools</p><h2 class="text-xl font-black">單張遮罩修補工作台</h2></div><div class="flex flex-wrap items-center gap-2"><button data-mask-tool="view" class="mask-tool h-9 rounded-xl bg-slate-100 px-3 text-xs font-black">檢視 / 平移</button><button data-mask-tool="magic" class="mask-tool h-9 rounded-xl bg-slate-100 px-3 text-xs font-black">魔術</button><button data-mask-tool="keep" class="mask-tool h-9 rounded-xl bg-slate-100 px-3 text-xs font-black">保留</button><button data-mask-tool="delete" class="mask-tool h-9 rounded-xl bg-slate-100 px-3 text-xs font-black">去背</button><button data-mask-tool="clear" class="mask-tool h-9 rounded-xl bg-slate-100 px-3 text-xs font-black">清除</button></div></div><div class="mt-4 grid items-start gap-3 rounded-3xl bg-slate-50 p-4 md:grid-cols-2 xl:grid-cols-4"><div><div class="text-xs font-black text-slate-500">魔術動作</div><div class="mt-2 grid grid-cols-2 gap-2"><button data-magic-action="delete" class="magic-action h-9 rounded-xl bg-white px-3 text-xs font-black">去背</button><button data-magic-action="keep" class="magic-action h-9 rounded-xl bg-white px-3 text-xs font-black">保留</button></div><label class="mt-2 flex h-5 items-center gap-2 text-xs font-black"><input id="magicContiguousInput" type="checkbox" ${state.settings.magicContiguous?'checked':''}>僅連續區域</label></div><div>${compactRangeInput('maskSizeInput','筆刷大小',state.settings.maskSize,5,120)}</div><div><label class="flex h-5 items-center justify-between text-xs font-black text-slate-500"><span>顯示遮罩標記</span><input id="maskOverlayInput" type="checkbox" ${state.settings.maskOverlayVisible?'checked':''}></label>${compactRangeInput('maskOverlayOpacityInput','遮罩透明度',state.settings.maskOverlayOpacity,5,100)}</div><div><div class="text-xs font-black text-slate-500">檢視模式</div><div class="mt-2 grid grid-cols-3 gap-1"><button data-refine-view="split" class="refine-view h-9 rounded-xl bg-white px-2 text-xs font-black">雙欄</button><button data-refine-view="mask" class="refine-view h-9 rounded-xl bg-white px-2 text-xs font-black">修補</button><button data-refine-view="result" class="refine-view h-9 rounded-xl bg-white px-2 text-xs font-black">成品</button></div></div></div><div class="mt-3 flex flex-wrap items-center gap-2"><button id="maskUndoBtn" class="h-9 rounded-xl bg-slate-100 px-3 text-xs font-black disabled:opacity-30">遮罩 Undo</button><button id="maskRedoBtn" class="h-9 rounded-xl bg-slate-100 px-3 text-xs font-black disabled:opacity-30">Redo</button><button id="maskClearBtn" class="h-9 rounded-xl bg-rose-50 px-3 text-xs font-black text-rose-700">清除全部標記</button><button id="resetSelectedRefineBtn" class="h-9 rounded-xl bg-amber-50 px-3 text-xs font-black text-amber-800">重設本張</button><button id="applyRefineBtn" class="h-9 rounded-xl bg-sky-600 px-3 text-xs font-black text-white">套用修補結果</button><span class="ml-auto self-center text-xs font-bold text-slate-400">滾輪縮放 · 拖曳平移 · 空白鍵暫時平移</span><button id="zoomOutBtn" class="h-9 rounded-xl bg-slate-100 px-3 text-xs font-black">-</button><button id="zoomResetBtn" class="h-9 rounded-xl bg-slate-100 px-3 text-xs font-black">${Math.round(state.refineView.zoom*100)}%</button><button id="zoomInBtn" class="h-9 rounded-xl bg-slate-100 px-3 text-xs font-black">+</button></div><div id="refineWorkspace" class="mt-4 grid gap-4 ${showEditor&&showResult?'lg:grid-cols-2':'grid-cols-1'}"><div id="refineViewport" class="relative min-h-[430px] overflow-hidden rounded-3xl border border-slate-200 bg-[linear-gradient(45deg,#e2e8f0_25%,transparent_25%),linear-gradient(-45deg,#e2e8f0_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#e2e8f0_75%),linear-gradient(-45deg,transparent_75%,#e2e8f0_75%)] bg-[length:18px_18px] touch-none" style="${showEditor?'':'display:none'};min-height:430px"><div id="refineTransformLayer" class="absolute left-1/2 top-1/2 origin-center" style="position:absolute;left:50%;top:50%;transform:${refineViewTransform(state.refineView)}"><canvas id="refineCanvas" class="block max-h-[70vh] max-w-[70vw] rounded-lg shadow-lg" style="display:block;max-width:760px;max-height:680px"></canvas></div><div id="refineBrushCursor" class="pointer-events-none absolute hidden rounded-full border-2 border-white shadow-[0_0_0_1px_rgba(15,23,42,.7)]"></div><div class="pointer-events-none absolute bottom-3 left-3 rounded-xl bg-slate-950/75 px-3 py-2 text-[10px] font-black text-white">第一筆保留 · 第二筆去背</div></div><div id="refineResultPane" class="grid min-h-[430px] place-items-center overflow-auto rounded-3xl border border-slate-200 bg-[linear-gradient(45deg,#e2e8f0_25%,transparent_25%),linear-gradient(-45deg,#e2e8f0_25%,transparent_25%)] bg-[length:18px_18px] p-5" style="${showResult?'':'display:none'};min-height:430px"><canvas id="refineOutputCanvas" class="max-h-[70vh] max-w-full rounded-lg shadow-lg"></canvas></div></div><div id="refineStatus" class="mt-4 rounded-2xl bg-slate-950 p-4 text-xs font-bold text-slate-200">尚未選取 Frame</div></section>`;
}

function renderReviewBoard() {
  return renderReviewLedBoard();
}
function renderReviewLedBoard() {
  const platformSpecs = getAvailablePlatformSpecs();
  const activeSpec = getPlatformSpec(state.settings.reviewPackCriterion);
  const activeAvailable = activeSpec.status === PlatformSpecStatuses.AVAILABLE;
  const stickerCounts = activeSpec.stickerCounts || [];
  const activeCount = normalizePlatformStickerCount(activeSpec.key, state.settings.reviewStickerCount);
  const includeMain = activeSpec.supportsMain !== false && state.settings.reviewIncludeMain !== false;
  const includeTab = activeSpec.supportsTab !== false && state.settings.reviewIncludeTab !== false;
  const specSummary = activeSpec.summary?.({ stickerCount: activeCount, main: includeMain, tab: includeTab }) || '';
  return `<section id="stage-review" class="scroll-mt-40 rounded-[1.75rem] border border-sky-200 bg-white p-5 shadow-sm">
    <div class="flex flex-wrap items-start justify-between gap-3"><div><p class="text-[10px] font-black uppercase tracking-[.2em] text-sky-600">Review · Package Planning</p><h2 class="text-xl font-black">貼圖編排與單張確認</h2><p class="mt-1 text-xs font-bold text-slate-400">先選交付規格與張數，再決定 main、tab、貼圖順序；超過數量的圖片會自動放入備選。</p></div><div class="flex flex-wrap gap-2"><button id="reviewPrevBtn" class="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">上一張</button><button id="reviewNextBtn" class="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">下一張</button><button id="toggleSafeGuideBtn" class="rounded-xl bg-rose-50 px-3 py-2 text-xs font-black text-rose-700">${state.settings.showSafeGuide ? '隱藏' : '顯示'}安全區</button><button id="toggleContentBoundsBtn" class="rounded-xl bg-sky-50 px-3 py-2 text-xs font-black text-sky-700">${state.settings.showContentBounds ? '隱藏' : '顯示'}內容邊界</button></div></div>
    <div class="mt-4 rounded-3xl border border-sky-100 bg-sky-50 p-4"><div class="flex flex-wrap items-start justify-between gap-3"><div><p class="text-[10px] font-black uppercase tracking-[.18em] text-sky-600">Platform Spec</p><h3 class="mt-1 text-lg font-black">交付平台規格</h3><p class="mt-1 text-xs font-bold text-slate-500">${escapeHtml(activeSpec.description)}</p><p class="mt-1 text-[10px] font-black uppercase tracking-widest ${activeAvailable ? 'text-sky-500' : 'text-amber-600'}">${escapeHtml(`${activeSpec.statusLabel || ''}${activeSpec.compatibilityLabel ? ` · ${activeSpec.compatibilityLabel}` : ''}`)}</p></div><button id="reviewApplyPackRuleBtn" class="h-11 rounded-2xl px-5 text-xs font-black ${activeAvailable ? 'bg-slate-950 text-white' : 'bg-slate-200 text-slate-500 cursor-not-allowed'}" ${activeAvailable ? '' : 'disabled'}>${activeAvailable ? '套用編排規格' : '即將支援，尚不能套用'}</button></div><div class="mt-4 grid gap-3 md:grid-cols-[220px_1fr_auto_auto]"><label class="text-xs font-black text-slate-600">平台規格<select id="reviewPackCriterionInput" class="mt-1 h-11 w-full rounded-2xl border border-sky-100 bg-white px-3 text-slate-950">${platformSpecs.map(spec => `<option value="${escapeHtml(spec.key)}" ${spec.key === activeSpec.key ? 'selected' : ''}>${escapeHtml(`${spec.label} · ${spec.statusLabel || ''}`)}</option>`).join('')}</select></label><div><div class="text-xs font-black text-slate-600">貼圖張數</div><select id="reviewStickerCountInput" class="mt-1 h-11 w-full rounded-2xl border border-sky-100 bg-white px-3 text-slate-950">${stickerCounts.map(count => `<option value="${count}" ${activeCount === count ? 'selected' : ''}>${count} 張貼圖</option>`).join('')}</select></div><label class="flex h-11 items-center gap-2 self-end rounded-2xl bg-white px-4 text-xs font-black text-slate-700"><input id="reviewIncludeMainInput" type="checkbox" ${activeSpec.supportsMain === false ? 'disabled' : ''} ${includeMain ? 'checked' : ''}>包含 Main</label><label class="flex h-11 items-center gap-2 self-end rounded-2xl bg-white px-4 text-xs font-black text-slate-700"><input id="reviewIncludeTabInput" type="checkbox" ${activeSpec.supportsTab === false ? 'disabled' : ''} ${includeTab ? 'checked' : ''}>包含 Tab</label></div><div id="reviewPackRuleSummary" class="mt-3 rounded-2xl bg-white px-4 py-3 text-xs font-black text-slate-600">${escapeHtml(specSummary)}</div></div>
    <div class="mt-4 flex flex-wrap items-end gap-3"><label class="min-w-[260px] flex-1 text-xs font-black text-slate-500">搜尋<input id="reviewSearchInput" value="${escapeHtml(state.settings.reviewSearch)}" placeholder="名稱、來源或檔名" class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-950"></label><label class="w-[170px] text-xs font-black text-slate-500">篩選<select id="reviewFilterInput" class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-950"><option value="all">全部</option><option value="errors">錯誤</option><option value="warnings">警告</option><option value="pending">待核准</option><option value="approved">已核准</option><option value="selected">匯出選取</option><option value="excluded">備選</option></select></label><label class="w-[190px] text-xs font-black text-slate-500">排序<select id="reviewSortInput" class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-950"><option value="frame-order">Frame 順序</option><option value="issue-severity">問題嚴重度</option><option value="file-size-desc">檔案大小</option><option value="name">名稱</option><option value="source">來源</option></select></label></div>
    <div class="mt-3 flex flex-wrap items-center gap-2"><span class="text-[10px] font-black uppercase tracking-widest text-slate-400">預覽背景</span><button data-review-bg="checker" class="review-bg rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">透明格</button><button data-review-bg="white" class="review-bg rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">白色</button><button data-review-bg="black" class="review-bg rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">黑色</button><button data-review-bg="sticker-preview" class="review-bg rounded-xl bg-[#06c755] px-3 py-2 text-xs font-black text-white">平台預覽</button><span class="ml-auto text-[10px] font-bold text-slate-400">滾輪縮放 · 拖曳平移 · 空白鍵暫時平移</span><button id="reviewZoomOutBtn" class="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">-</button><button id="reviewZoomResetBtn" class="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">${Math.round(state.reviewView.zoom * 100)}%</button><button id="reviewZoomInBtn" class="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black">+</button></div>
    <div class="mt-4 grid gap-4 lg:grid-cols-[1fr_250px]"><div id="reviewHeroStage" class="relative min-h-[360px] overflow-hidden rounded-3xl border border-slate-200 touch-none" style="position:relative;min-height:360px;overflow:hidden;touch-action:none"><div id="reviewTransformLayer" class="pointer-events-none absolute left-1/2 top-1/2 h-full w-full origin-center" style="position:absolute;left:50%;top:50%;width:100%;height:100%;pointer-events:none;transform-origin:center;transform:${refineViewTransform(state.reviewView)}"><img id="reviewHeroImage" class="absolute inset-0 h-full w-full object-contain" style="position:absolute;inset:0;width:100%;height:100%;object-fit:contain;pointer-events:none"><div id="reviewSafeGuide" class="pointer-events-none absolute border-2 border-rose-500 bg-rose-500/10" style="position:absolute;pointer-events:none"></div><div id="reviewContentBounds" class="pointer-events-none absolute border-2 border-sky-400 bg-sky-400/10" style="position:absolute;pointer-events:none"></div></div></div><div id="reviewHeroMeta" class="rounded-3xl bg-slate-950 p-4 text-sm text-white"></div></div>
    <div class="mt-4 flex flex-wrap items-center gap-3"><div class="flex flex-wrap items-center gap-2 rounded-2xl bg-slate-50 p-2"><span class="px-2 text-[10px] font-black uppercase tracking-widest text-slate-400">選取</span><button id="reviewSelectAllBtn" class="rounded-xl bg-white px-3 py-2 text-xs font-black">全選</button><button id="reviewSelectNoneBtn" class="rounded-xl bg-white px-3 py-2 text-xs font-black">排除</button><button id="reviewInvertBtn" class="rounded-xl bg-white px-3 py-2 text-xs font-black">反選</button></div><div class="flex flex-wrap items-center gap-2 rounded-2xl bg-slate-50 p-2"><span class="px-2 text-[10px] font-black uppercase tracking-widest text-slate-400">核准</span><button id="reviewApproveCleanBtn" class="rounded-xl bg-emerald-300 px-3 py-2 text-xs font-black text-slate-950">核准無錯誤項目</button><button id="reviewRevokeVisibleBtn" class="rounded-xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-800">撤銷核准</button></div><div id="reviewNextStepHint" class="ml-auto hidden rounded-2xl bg-emerald-50 px-4 py-3 text-xs font-black text-emerald-800"><span>Review 完成後，Package 只做交付確認。</span><button id="reviewGoPackageBtn" class="ml-3 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white">前往交付確認</button></div></div>
    <div id="reviewProgressBar" class="mt-4"></div><div id="reviewGrid" class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 2xl:grid-cols-5"></div>
  </section>`;
}
...
function renderDeliveryConfirmBoard() {
  return '<div id="packageWorkspaceRoot"></div>';
}

function renderSourceListPanel() {
  return `<section class="rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><p class="text-[10px] font-black uppercase tracking-[.2em] text-emerald-600">Layout 蝜?Document Engine</p><h2 class="mt-1 text-xl font-black">?賹??謘?/h2><div id="sourceList" class="mt-4 space-y-2"></div></section>`;
}

function renderSelectedPanel() {
  const frame = selectedFrame();
  return `<section class="rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><p class="text-[10px] font-black uppercase tracking-[.2em] text-emerald-600">Layout · Selected Frame</p><div id="selectedInfo" class="mt-4 text-sm text-slate-500">尚未選取 Frame</div><div class="mt-4 grid grid-cols-2 gap-2">${numberInput('offsetXInput','Offset X',frame?.custom?.offsetX||0,-4096,4096)}${numberInput('offsetYInput','Offset Y',frame?.custom?.offsetY||0,-4096,4096)}</div><div class="mt-2 grid grid-cols-4 gap-2"><button data-nudge-x="-1" class="nudge-btn rounded-xl bg-slate-100 py-2 text-xs font-black">←</button><button data-nudge-y="-1" class="nudge-btn rounded-xl bg-slate-100 py-2 text-xs font-black">↑</button><button data-nudge-y="1" class="nudge-btn rounded-xl bg-slate-100 py-2 text-xs font-black">↓</button><button data-nudge-x="1" class="nudge-btn rounded-xl bg-slate-100 py-2 text-xs font-black">→</button></div><button id="resetOffsetBtn" class="mt-2 w-full rounded-xl bg-emerald-50 py-2 text-xs font-black text-emerald-800">重設輸出位移</button><div class="mt-4 grid grid-cols-2 gap-2"><button id="duplicateBtn" class="rounded-xl bg-slate-100 py-2 text-xs font-black">複製</button><button id="deleteBtn" class="rounded-xl bg-rose-50 py-2 text-xs font-black text-rose-700">刪除</button></div></section>`;
}
function renderReviewDecisionPanel() {
  return `<section class="rounded-[1.75rem] border border-sky-200 bg-sky-950 p-5 text-white shadow-sm">
    <p class="text-[10px] font-black uppercase tracking-[.2em] text-sky-300">Review 蝜?Decision Center</p>
    <h2 class="mt-1 text-lg font-black">???????/h2>
    <div id="reviewGateStatus" class="mt-4"></div>
    <div class="mt-3 grid grid-cols-2 gap-2">
      <button id="reviewApproveCurrentBtn" class="rounded-xl bg-emerald-300 px-3 py-2 text-xs font-black text-slate-950">?詨??獢??/button>
      <button id="reviewRevokeCurrentBtn" class="rounded-xl bg-white/10 px-3 py-2 text-xs font-black">??伐??獢??/button>
    </div>
    <button id="reviewNextIssueBtn" class="mt-2 w-full rounded-xl bg-sky-500/20 px-3 py-2 text-xs font-black text-sky-100">?鈭?????????/button>
    <div id="reviewIssueList" class="mt-4 max-h-[360px] space-y-2 overflow-auto"></div>
  </section>`;
}

function renderDeliveryConfirmPanel() {
...
function compactRangeInput(id,label,value,min,max){return `<label class="block text-xs font-black text-slate-500"><span class="flex h-5 items-center justify-between"><span>${label}</span><span id="${id}Value">${value}</span></span><input id="${id}" type="range" min="${min}" max="${max}" value="${value}" class="mt-2 w-full accent-emerald-500"></label>`;}
...
function bindStaticEvents(root) {
  ...
  root.querySelector('#manualLayoutToggle')?.addEventListener('click', toggleManualLayoutSettings);
  ...
  root.querySelector('#toggleSafeGuideBtn')?.addEventListener('click',()=>{state.settings.showSafeGuide=!state.settings.showSafeGuide;renderLargeReview();const button=root.querySelector('#toggleSafeGuideBtn');if(button)button.textContent=(state.settings.showSafeGuide?'隱藏':'顯示')+'安全區';});
  root.querySelector('#toggleContentBoundsBtn')?.addEventListener('click',()=>{state.settings.showContentBounds=!state.settings.showContentBounds;renderLargeReview();const button=root.querySelector('#toggleContentBoundsBtn');if(button)button.textContent=(state.settings.showContentBounds?'隱藏':'顯示')+'內容邊界';});
  ...
  root.querySelector('#reviewPackCriterionInput')?.addEventListener('change',event=>{state.settings.reviewPackCriterion=event.target.value;syncReviewPackRuleControls();state.packageController?.refresh?.();});
  root.querySelector('#reviewStickerCountInput')?.addEventListener('change',event=>{state.settings.reviewStickerCount=Number(event.target.value)||8;syncReviewPackRuleControls();state.packageController?.refresh?.();});
  root.querySelector('#reviewIncludeMainInput')?.addEventListener('change',event=>{state.settings.reviewIncludeMain=event.target.checked;syncReviewPackRuleControls();state.packageController?.refresh?.();});
  root.querySelector('#reviewIncludeTabInput')?.addEventListener('change',event=>{state.settings.reviewIncludeTab=event.target.checked;syncReviewPackRuleControls();state.packageController?.refresh?.();});
  root.querySelector('#reviewApplyPackRuleBtn')?.addEventListener('click',applyReviewPackRule);
  ...
  root.querySelector('#reviewGoPackageBtn')?.addEventListener('click',()=>document.getElementById('stage-package')?.scrollIntoView({behavior:'smooth',block:'start'}));
...
function mountPackageController(root){
  if(!state.packageController){
    state.packageController=createPackageController({
      ...
      getPlatformSpec:()=>getPlatformSpec(state.settings.reviewPackCriterion),
      ...
      assignRoles:mode=>{
        ...
        if(mode==='auto'&&selected.length<required.length+minimumSticker){window.alert(`目前規格至少需要 ${required.length+minimumSticker} 張可交付圖片。`);return false;}
      ...
      openReview:()=>{state.activeEditor='review';refresh();document.getElementById('stage-review')?.scrollIntoView({behavior:'smooth',block:'start'});},
...
function bindRange(root,id,key,render=true,afterChange=null){const input=root.querySelector(`#${id}`);if(!input)return;input.addEventListener('input',event=>{state.settings[key]=Number(event.target.value);const label=root.querySelector(`#${id}Value`);if(label)label.textContent=event.target.value;if(render){renderSelectedLowQualityPreview();scheduleRenderAll();}if(afterChange)afterChange();});if(render)input.addEventListener('change',()=>scheduleRenderAll(20));}
function updateRefineSetting(key,value){state.settings[key]=value;renderSelectedLowQualityPreview();scheduleRenderAll();}
function setPreviewInteractionActive(active){state.previewInteractionActive=active;const button=document.getElementById('exportZipBtn');if(button){button.disabled=active;button.classList.toggle('opacity-40',active);button.title=active?'甇??湔?汗嚗?蝔?頛詨':'';}}
...
function toggleManualLayoutSettings(){state.settings.manualLayoutOpen=!state.settings.manualLayoutOpen;saveActiveSourceLayout();syncManualLayoutControls();}
function setLayoutMode(mode){state.settings.layoutMode=mode;if(mode==='auto')state.settings.manualLayoutOpen=false;else state.settings.manualLayoutOpen=true;if(mode==='1x1'){state.settings.rows=1;state.settings.cols=1;}if(mode==='2x2'){state.settings.rows=2;state.settings.cols=2;}if(mode==='3x3'){state.settings.rows=3;state.settings.cols=3;}saveActiveSourceLayout();syncShellControls();refreshLayoutSection();}
...
async function importFiles(fileList) {
  ...
  for (const file of files) {
    ...
    const importLayout=createSourceLayoutSettings({...state.settings,layoutMode:'auto',rows:1,cols:1,marginX:0,marginY:0,gapX:0,gapY:0,manualLayoutOpen:false});
    state.sourceLayouts.set(ref.id,importLayout);
    state.document = addSourceRef(state.document,ref);
    state.activeSourceId = ref.id;
    state.settings=applySourceLayoutSettings(state.settings,importLayout);
    detectSource(ref.id);
  }
...
function deleteSource(sourceId){
  ...
  if(!window.confirm(`刪除 ${source.name}？這會移除這個來源與相關 Frame。`))return;
...
function setFrames(next){state.document=setDocumentFrames(state.document,next);invalidateReviewCaches();state.projectController?.markDirty('Frames updated');}
...
function refresh(){
  if(state.sources.size===0){
    ...
    const sourceList=document.getElementById('sourceList');if(sourceList)sourceList.innerHTML='<div class="rounded-2xl bg-slate-50 p-4 text-sm text-slate-400">?垓??賹?</div>';
    const selected=document.getElementById('selectedInfo');if(selected)selected.textContent='尚未選取 Frame';
    const grid=document.getElementById('reviewGrid');if(grid)grid.innerHTML='<div class="col-span-full rounded-3xl border border-dashed border-slate-300 p-12 text-center text-slate-400">?垓??瞉?</div>';
...
function renderEmptyReviewState(){
  ...
  if(summary)summary.innerHTML='<div class="rounded-2xl bg-white/10 p-3 text-xs text-slate-400">?垓?????鈭亦巨??/div>';

  const gate=document.getElementById('reviewGateStatus');
  if(gate)gate.innerHTML='<div class="rounded-2xl bg-white/10 p-3"><div class="text-2xl font-black">0/0</div><div class="text-xs">?垮謓??穿???</div></div>';

  const issues=document.getElementById('reviewIssueList');
  if(issues)issues.innerHTML='<div class="rounded-xl bg-white/10 p-3 text-xs text-slate-300">????潘撓貔??梱?</div>';

  const progress=document.getElementById('reviewProgressBar');
  if(progress)progress.innerHTML='<div class="flex items-center justify-between text-xs font-black"><span>?赯??0/0</span><span>0%</span></div><div class="mt-2 h-3 overflow-hidden rounded-full bg-slate-100"></div>';
...
function refreshCommonControls(){
  ...
  if(status){
    ...
    status.textContent=source?`${source.name} 蝜?${frames().filter(frame=>frame.sourceImageId===source.id).length} Frames`:'?垮謓??穿';
  }
...
function renderSourceList(){
  ...
  if(!state.sources.size){holder.innerHTML='<div class="rounded-2xl bg-slate-50 p-4 text-sm text-slate-400">?垓??賹?</div>';return;}
  holder.innerHTML='';
...
function bindSourceCanvas(canvas){if(!canvas)return;canvas.addEventListener('pointerdown',sourcePointerDown);canvas.addEventListener('pointermove',sourcePointerMove);canvas.addEventListener('pointerup',sourcePointerUp);canvas.addEventListener('pointerleave',sourcePointerUp);}
...
function drawRefineCanvas(updateOutput=true){
  ...
  if(!frame||!source){canvas.width=1;canvas.height=1;ctx.clearRect(0,0,1,1);if(outputCanvas){outputCanvas.width=1;outputCanvas.height=1;outputCanvas.getContext('2d').clearRect(0,0,1,1);}if(status)status.textContent='?垮謓舀??Frame';return;}
  ...
  if(status)status.innerHTML=`<span class="text-emerald-300">${escapeHtml(frame.name)}</span> 蝜??踐?? ${stats.keep}px 蝜???畸? ${stats.delete}px 蝜???? ${(stats.coverage*100).toFixed(1)}% 蝜???? ${kb}KB${applied?` 蝜?????${new Date(applied).toLocaleTimeString()}`:''}`;
}
function bindRefineCanvas(canvas){if(!canvas)return;canvas.addEventListener('pointerdown',refinePointerDown);canvas.addEventListener('pointermove',refinePointerMove);canvas.addEventListener('pointerup',refinePointerUp);canvas.addEventListener('pointercancel',refinePointerUp);canvas.addEventListener('pointerleave',event=>{updateBrushCursor();if(!state.maskStroke)state.refinePointer=null;});}
...
function installProjectDirtyEventTracking(root){
  ...
  const markDirty=event=>{
    ...
    state.projectController?.markDirty('Settings updated');
  };
...
function roleLabel(role){const profile=state.destinationController?.getActiveProfile?.();return profile?.roles.find(item=>item.key===role)?.label||({sticker:'Sticker',main:'Main',tab:'Tab',background:'Background','effect-background':'Effect Background'})[role]||role;}
...
function approveVisibleClean(){const ids=visibleReviewItems().filter(item=>!item.hasErrors).map(item=>item.frameId);if(!ids.length)return;setFrames(setFramesReviewApproval(frames(),ids,true));runReview();refresh();}
function revokeVisibleApproval(){const ids=visibleReviewItems().map(item=>item.frameId);if(!ids.length)return;setFrames(setFramesReviewApproval(frames(),ids,false));runReview();refresh();}
function navigateReview(direction=1){const nextId=nextReviewFrameId(allReviewItems(),state.selectedFrameId,direction);if(!nextId)return;selectFrame(nextId);state.activeEditor='review';refresh();}
function setCurrentReviewApproval(approved){const frame=selectedFrame();if(!frame)return;replaceFrame(setFrameReviewApproval(frame,approved),{preserveReview:true});runReview();refresh();}
function toggleFrameReviewApproval(frameId){const frame=frames().find(item=>item.id===frameId);if(!frame)return;replaceFrame(setFrameReviewApproval(frame,!frame.state?.reviewApproved),{preserveReview:true});runReview();refresh();}
function navigateNextIssue(){const issue=runReview().issues.find(item=>item.code!=='review.pendingApproval'&&item.frameId);if(!issue?.frameId)return;navigateToReviewFrame(issue.frameId);}
function navigateToReviewFrame(frameId){selectFrame(frameId);state.activeEditor='review';resetReviewViewport();refresh();document.getElementById('stage-review')?.scrollIntoView({behavior:'smooth',block:'start'});}
function reviewPackRuleSlots(){
  const spec=getPlatformSpec(state.settings.reviewPackCriterion);
  const stickerCount=normalizePlatformStickerCount(spec.key,state.settings.reviewStickerCount);
  const main=spec.supportsMain!==false&&state.settings.reviewIncludeMain!==false;
  const tab=spec.supportsTab!==false&&state.settings.reviewIncludeTab!==false;
  return {spec,stickerCount,main,tab,total:stickerCount+(main?1:0)+(tab?1:0)};
}
function syncReviewPackRuleControls(){
  const criterion=document.getElementById('reviewPackCriterionInput'),count=document.getElementById('reviewStickerCountInput'),main=document.getElementById('reviewIncludeMainInput'),tab=document.getElementById('reviewIncludeTabInput'),summary=document.getElementById('reviewPackRuleSummary'),applyButton=document.getElementById('reviewApplyPackRuleBtn');
  const slots=reviewPackRuleSlots();
  const available=slots.spec.status===PlatformSpecStatuses.AVAILABLE;
  if(criterion)criterion.value=slots.spec.key;
  if(count)count.value=String(slots.stickerCount);
  if(main){main.disabled=slots.spec.supportsMain===false;main.checked=slots.main;}
  if(tab){tab.disabled=slots.spec.supportsTab===false;tab.checked=slots.tab;}
  if(summary)summary.textContent=slots.spec.summary?.(slots)||`交付規格會使用 ${slots.total} 張；超過數量會放入備選。`;
  if(applyButton){applyButton.disabled=!available;applyButton.textContent=available?'套用編排規格':'即將支援，尚不能套用';applyButton.classList.toggle('cursor-not-allowed',!available);applyButton.classList.toggle('bg-slate-950',available);applyButton.classList.toggle('text-white',available);applyButton.classList.toggle('bg-slate-200',!available);applyButton.classList.toggle('text-slate-500',!available);}
}
function applyReviewPackRule(){
  const slots=reviewPackRuleSlots();
  const spec=slots.spec;
  if(spec.status!==PlatformSpecStatuses.AVAILABLE){window.alert(`${spec.label} 目前是「${spec.statusLabel}」，尚不能套用。`);syncReviewPackRuleControls();return;}
  state.settings.reviewPackCriterion=spec.key;
  state.settings.reviewStickerCount=slots.stickerCount;
  state.settings.reviewSort=ReviewSortModes.FRAME_ORDER;
  state.settings.reviewFilter=ReviewFilterModes.ALL;
  state.settings.packageNamingMode=PackageNamingModes.PACKAGE;
  const destinationState=state.destinationController?.exportState?.()||{};
  state.destinationController?.importState?.({...destinationState,activeKey:spec.destinationProfileKey,activeRole:spec.destinationRole||AssetRoles.STICKER});
  state.settings=applyDestinationProfileToSettings(state.settings,getBuiltInDestinationProfile(spec.destinationProfileKey),spec.destinationRole||AssetRoles.STICKER);
  setFrames(applyPlatformStickerReviewRule(frames(),spec.key,{stickerCount:slots.stickerCount,includeMain:slots.main,includeTab:slots.tab}));
  clearRenderCache();renderAll();runReview();refresh();
}
...
function renderReviewGrid(){
  ...
  if(!frames().length){grid.innerHTML='<div class="col-span-full rounded-3xl border border-dashed border-slate-300 p-12 text-center text-slate-400">尚未產生 Frame</div>';return;}
  const items=visibleReviewItems();
  if(!items.length){grid.innerHTML='<div class="col-span-full rounded-3xl border border-dashed border-slate-300 p-12 text-center text-slate-400">目前篩選沒有符合的 Frame</div>';return;}
  ...
  virtual.items.forEach(item=>{const frame=item.frame,canvas=renderFrame(frame),card=document.createElement('article');card.draggable=canReorder;card.dataset.frameId=frame.id;card.dataset.sourceId=frame.sourceImageId;card.dataset.reviewCard='true';card.dataset.reviewSeverity=item.highestSeverity||'clean';card.dataset.reviewApproved=String(item.approved);card.dataset.exportSelected=String(item.exportSelected);const severityClass=item.hasErrors?'border-rose-500 bg-rose-50':item.hasWarnings?'border-amber-400 bg-amber-50':item.approved?'border-emerald-400 bg-emerald-50':frame.id===state.selectedFrameId?'border-sky-400 bg-sky-50':'border-slate-200 bg-white';card.className=`flex min-h-[252px] flex-col rounded-3xl border p-2 transition ${severityClass}`;const badge=item.hasErrors?'錯誤':item.hasWarnings?'警告':item.approved?'已核准':'待核准';const fileName=item.fileName||'備選';card.innerHTML=`<button class="preview relative block aspect-[370/320] w-full overflow-hidden rounded-2xl"><img class="h-full w-full object-contain" alt=""><span class="absolute left-2 top-2 rounded-full bg-slate-950/80 px-2 py-1 text-[10px] font-black text-white">${badge}</span><label class="absolute right-2 top-2 flex items-center rounded-full bg-white/90 px-2 py-1 text-[10px] font-black shadow-sm"><input class="export-check mr-1" type="checkbox" ${item.exportSelected?'checked':''}>匯出</label></button><div class="mt-2 flex items-center justify-between gap-2"><span class="text-xs font-black">#${item.index+1} · ${item.kb}KB</span><span class="rounded-full ${item.approved?'bg-emerald-100 text-emerald-700':item.hasErrors?'bg-rose-100 text-rose-700':item.hasWarnings?'bg-amber-100 text-amber-700':'bg-slate-100 text-slate-500'} px-2 py-1 text-[10px] font-black">${badge}</span></div><div class="mt-1 truncate text-[10px] text-slate-400" title="${escapeHtml(item.sourceName)}">${escapeHtml(item.sourceName)}</div><div class="mt-auto grid min-h-[78px] gap-2 pt-2"><div class="truncate text-[10px] text-slate-400" title="${escapeHtml(fileName)}">${escapeHtml(fileName)}</div><div class="flex gap-2"><button class="review-approve flex-1 rounded-xl ${item.approved?'bg-emerald-500 text-white':'bg-slate-100'} py-2 text-xs font-black">${item.approved?'撤銷核准':'核准'}</button><button class="single-download rounded-xl bg-slate-950 px-3 py-2 text-xs font-black text-white">單張 PNG</button></div><select class="role-select w-full rounded-xl bg-slate-100 px-2 py-1 text-xs font-black" title="輸出角色">${roles.map(role=>`<option value="${role}" ${packageRole(frame)===role?'selected':''}>${roleLabel(role)}</option>`).join('')}</select></div>`;
    ...
    card.addEventListener('dragstart',event=>{state.draggedReviewFrameId=frame.id;if(event.dataTransfer){event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',frame.id);}});card.addEventListener('dragenter',event=>{if(canReorder)event.preventDefault();});card.addEventListener('dragover',event=>{if(canReorder){event.preventDefault();if(event.dataTransfer)event.dataTransfer.dropEffect='move';}});card.addEventListener('drop',event=>{if(!canReorder)return;event.preventDefault();const fromId=event.dataTransfer?.getData('text/plain')||state.draggedReviewFrameId;reorderFrame(fromId,frame.id);});
    card.addEventListener('dragend',event=>{const fromId=state.draggedReviewFrameId;state.draggedReviewFrameId=null;if(!canReorder||!fromId)return;const element=document.elementFromPoint(event.clientX,event.clientY);const target=element&&element.closest('[data-review-card="true"]');const toId=target&&target.dataset.frameId;if(toId&&toId!==fromId)reorderFrame(fromId,toId);});grid.appendChild(card);
  });
...
function renderLargeReview(){
  ...
  if(!frame){image.removeAttribute('src');guide.classList.add('hidden');bounds.classList.add('hidden');meta.innerHTML='<div class="text-slate-400">尚未選取預覽</div>';return;}
  ...
  const boundsText=analysis.bounds?`${analysis.bounds.width}x${analysis.bounds.height}`:'none';
  meta.innerHTML=`<div class="text-xs font-black uppercase tracking-widest text-sky-300">Selected Review</div><div class="mt-2 break-all text-lg font-black">${escapeHtml(frame.name)}</div><div class="mt-1 break-all font-mono text-xs text-slate-300">${escapeHtml(item?.fileName||'')}</div><div class="mt-3 rounded-2xl bg-white/10 p-3">${roleLabel(packageRole(frame))}<br>${output.width}x${output.height}px<br>safe ${output.safeMargin}px<br>內容 ${boundsText}<br>透明 ${Math.round((analysis.transparentRatio||0)*100)}%<br>${item?.kb||0}KB</div><button id="reviewHeroApproveBtn" class="mt-3 w-full rounded-xl ${item?.approved?'bg-emerald-500':'bg-emerald-300 text-slate-950'} py-2 text-xs font-black">${item?.approved?'撤銷核准':'核准目前 Frame'}</button><label class="mt-2 flex items-center justify-between rounded-xl bg-white/10 p-2 text-xs font-black"><span>包含在匯出</span><input id="reviewHeroExportInput" type="checkbox" ${item?.exportSelected?'checked':''}></label>${issueBadges||'<div class="mt-3 rounded-xl bg-emerald-500/20 p-2 text-xs text-emerald-200">沒有發現錯誤</div>'}`;
...
function reviewIssueText(issue){const labels={'render.blank':'內容可能是空白','render.lowContent':'內容比例偏低','render.edgeTouch':'內容碰到邊界','render.safeMargin':'內容進入安全邊距','render.opaqueBackground':'背景不透明','render.fileTooLarge':'檔案過大','package.duplicateFilename':'輸出檔名重複','review.pendingApproval':'尚未核准','render.sizeMismatch':'輸出尺寸不符','render.missing':'缺少輸出圖','destination.role.exact':'角色數量不符','destination.role.min':'角色數量不足','destination.role.max':'角色數量過多','destination.role.allowedCount':'貼圖張數不符合規格','destination.output.size':'輸出尺寸不符合規格','destination.output.fileSize':'檔案大小超過規格','destination.package.empty':'沒有可交付項目'};return labels[issue.code]||issue.message||issue.code;}
function renderReviewProgress(){const holder=document.getElementById('reviewProgressBar');if(!holder)return;const progress=getReviewProgress(allReviewItems());holder.innerHTML=`<div class="flex flex-wrap items-center gap-2 text-xs font-black"><span>${progress.approved}/${progress.selected} 已核准</span><span class="text-slate-300">·</span><span>${progress.withErrors} 錯誤</span><span class="text-slate-300">·</span><span>${progress.withWarnings} 警告</span><span class="text-slate-300">·</span><span>${progress.excluded} 備選</span><span class="ml-auto text-slate-500">${progress.percent}%</span></div><div class="mt-2 h-3 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full ${progress.ready?'bg-emerald-500':'bg-sky-500'}" style="width:${progress.percent}%"></div></div>`;}
function renderReviewInspector(){const gate=document.getElementById('reviewGateStatus'),list=document.getElementById('reviewIssueList');if(!gate||!list)return;runReview();const report=state.reviewReport,progress=getReviewProgress(allReviewItems());gate.innerHTML=`<div class="rounded-2xl ${report.canPackage?'bg-emerald-500/20 text-emerald-200':'bg-white/10'} p-3"><div class="text-2xl font-black">${progress.approved}/${progress.selected}</div><div class="text-xs">已核准 · ${report.summary.errors} errors · ${report.summary.warnings} warnings</div><div class="mt-2 text-xs font-black">${report.canPackage?'Review 完成，可以前往 Package':'尚有項目需要確認'}</div></div>`;const issues=report.issues.filter(issue=>issue.code!=='review.pendingApproval');if(!issues.length){list.innerHTML='<div class="rounded-xl bg-emerald-500/20 p-3 text-xs text-emerald-200">沒有發現品質問題</div>';return;}list.innerHTML=issues.map(issue=>`<button data-review-issue-frame="${issue.frameId||''}" class="block w-full rounded-xl ${issue.severity==='error'?'bg-rose-500/20 text-rose-100':issue.severity==='warning'?'bg-amber-500/20 text-amber-100':'bg-white/10'} p-3 text-left text-xs"><span class="font-black">${issue.severity.toUpperCase()}</span><br>${escapeHtml(reviewIssueText(issue))}</button>`).join('');list.querySelectorAll('[data-review-issue-frame]').forEach(button=>button.addEventListener('click',()=>{if(button.dataset.reviewIssueFrame){selectFrame(button.dataset.reviewIssueFrame);state.activeEditor='review';resetReviewViewport();refresh();document.getElementById('stage-review')?.scrollIntoView({behavior:'smooth',block:'start'});}}));}
function renderSelectedInfo(){const holder=document.getElementById('selectedInfo'),frame=selectedFrame();if(!holder)return;if(!frame){holder.textContent='尚未選取 Frame';return;}const g=frame.geometry,ox=frame.custom?.offsetX||0,oy=frame.custom?.offsetY||0;holder.innerHTML=`<div class="rounded-2xl bg-emerald-50 p-3 font-black text-emerald-800">${escapeHtml(frame.name)}<br><span class="text-xs">x ${Math.round(g.x)} · y ${Math.round(g.y)} · ${Math.round(g.width)}×${Math.round(g.height)}<br>output offset: ${ox}, ${oy}<br>role: ${roleLabel(packageRole(frame))}</span></div>`;const x=document.getElementById('offsetXInput'),y=document.getElementById('offsetYInput');if(x)x.value=ox;if(y)y.value=oy;}
function renderReviewSummary(){const holder=document.getElementById('reviewSummary');if(!holder)return;const report=runReview(),plan=report?.packagePlan||packagePlan(exportFrames());if(!report){holder.innerHTML='<div class="text-slate-400">尚未建立 Review</div>';return;}holder.innerHTML=`<div class="rounded-2xl bg-white/10 p-3"><div class="text-2xl font-black">${exportFrames().length}</div><div class="text-xs text-slate-400">selected exports</div></div><div class="rounded-2xl bg-white/10 p-3 text-xs">${report.ready&&plan.ready?'Review ready':`${report.summary.errors+plan.validation.errors.length} errors · ${report.summary.warnings} warnings`}</div>`;}
...
async function createRuntimeSource(source){
  ...
  if(!blob)throw new Error(`????謘??餌?隞??{source.name||source.id}`);
