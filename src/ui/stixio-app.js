      ...
      <header class="sticky top-0 z-30 border-b border-slate-900/10 bg-[#f6f3ec]/90 backdrop-blur-xl">
        <div class="mx-auto max-w-[1500px] px-5 py-4 flex items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="flex items-center gap-2"><h1 class="text-xl font-black tracking-tight">${BRAND.name}</h1><span class="rounded-full border border-slate-900/10 bg-white/70 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-slate-500">Beta</span></div>
          ...
          <div class="hidden lg:flex items-center gap-2 rounded-2xl border border-slate-900/10 bg-white/70 p-1 shadow-sm">
            ${WORKSPACES.map((label, index) => `<button class="workspace-tab rounded-xl px-4 py-2 text-left transition ${index === 3 ? 'bg-slate-950 text-white shadow' : 'hover:bg-slate-100'}"><span class="block text-[10px] font-black uppercase tracking-widest ${index === 3 ? 'text-emerald-300' : 'text-slate-400'}">0${index + 1}</span><span class="block text-sm font-black">${label}</span></button>`).join('')}
          </div>
          <div class="flex items-center gap-2"><span class="hidden md:inline-flex rounded-xl border border-slate-900/10 bg-white/70 px-3 py-2 text-xs font-black text-slate-700">Core Editor</span><button id="workspaceCtaBtn" class="rounded-xl bg-slate-950 px-4 py-2 text-xs font-black text-white shadow hover:-translate-y-0.5 transition">M1 Only</button></div>
        </div>
      </header>
  ...
  return `
    <section class="relative overflow-hidden rounded-[2rem] bg-slate-950 text-white p-6 md:p-8 shadow-xl">
      ...
      <div class="relative grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-8 items-center">
        <div>
          <div class="mb-4 inline-flex rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs font-black text-emerald-200">Review + Export Engine</div>
          <h2 class="text-4xl md:text-6xl font-black leading-tight tracking-tight">Create once.<br><span class="text-emerald-300">Export safely.</span></h2>
          <p class="mt-4 max-w-2xl text-sm md:text-base text-slate-300">Review now checks rendered Frames before export. PNG and ZIP now go through Export Engine.</p>
        ...
        <div class="rounded-[1.5rem] border border-white/10 bg-white/10 p-4 backdrop-blur"><div class="grid grid-cols-2 gap-3">${metricCard('Frames', state.frames.length || '0', 'detected areas')}${metricCard('Quality', getDetectionScoreLabel(), 'grid confidence')}${metricCard('Rendered', state.rendered.size || '0', 'cached previews')}${metricCard('Review', getReviewSummaryLabel(), 'export readiness')}</div></div>
      </div>
    </section>
  `;
}
...
function renderLeftPanel() {
  return `
    <aside class="space-y-4">
      <section class="rounded-[1.75rem] border border-slate-900/10 bg-white p-5 shadow-sm"><div class="flex items-start justify-between gap-3"><div><p class="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600">Quick Project</p><h3 class="mt-1 text-xl font-black">Import artwork</h3></div><span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">OPEN</span></div><p class="mt-3 text-sm text-slate-500">Upload a sheet. Detection creates Frames; Review validates output before Export.</p><label id="dropZone" class="mt-5 block cursor-pointer rounded-3xl border-2 border-dashed border-slate-300 bg-slate-50 p-6 text-center transition hover:border-emerald-400 hover:bg-emerald-50"><input id="fileInput" type="file" accept="image/*" class="hidden" /><div class="mx-auto mb-3 grid h-14 w-14 place-items-center rounded-2xl bg-slate-950 text-3xl text-emerald-300">＋</div><p class="font-black">Drop or choose artwork</p><p class="mt-1 text-xs text-slate-500">PNG, JPG, WebP. One sheet is enough.</p></label></section>
...
function renderReviewIssues() {
  if (!state.reviewReport) return healthRow(false, 'No review yet');
  if (!state.reviewReport.issues.length) return healthRow(true, 'No review issues');
  return state.reviewReport.issues.slice(0, 8).map(issue => healthRow(issue.severity !== 'error', issue.message)).join('');
}
...
function getReviewSummaryLabel() {
  if (!state.reviewReport) return 'Not reviewed';
  const { errors, warnings } = state.reviewReport.summary;
  return state.reviewReport.ready ? `Ready · ${warnings} warnings` : `${errors} errors`;
}
...
function calculateHealth() {
  ...
  if (state.reviewReport?.ready) score += 25;
  return score;
}
...
function createFramesFromSource() {
  ...
  state.detectionReport = detectGrid(state.source, { grid: { rows: state.settings.rows, cols: state.settings.cols, marginX: state.settings.marginX, marginY: state.settings.marginY, gapX: state.settings.gapX, gapY: state.settings.gapY, snapToPixels: true, minCellSize: 8 } });
...
function drawSourcePreview() {
  ...
  state.frames.forEach(frame => { const selected = frame.id === state.settings.selectedId; ctx.strokeStyle = selected ? '#10b981' : '#ef4444'; ctx.lineWidth = selected ? Math.max(6, canvas.width / 180) : Math.max(3, canvas.width / 300); ctx.strokeRect(frame.geometry.x, frame.geometry.y, frame.geometry.width, frame.geometry.height); });
}

function renderFrameGrid() {
  const grid = document.getElementById('frameGrid');
  if (!grid) return;
  if (!state.frames.length) { grid.innerHTML = '<div class="col-span-full rounded-[1.5rem] border border-dashed border-slate-300 bg-slate-50 p-12 text-center"><p class="text-lg font-black text-slate-700">No Frames yet</p><p class="mt-1 text-sm text-slate-500">Import artwork and run detection to start.</p></div>'; return; }
  grid.innerHTML = '';
  state.frames.forEach((frame, index) => {
    const canvas = state.rendered.get(frame.id) || renderFrame(frame);
    const card = document.createElement('button');
    const bg = getReviewCardBackgroundClass();
...
function getReviewCardBackgroundClass() {
  if (state.settings.reviewBackground === ReviewBackgrounds.WHITE) return 'bg-white';
  if (state.settings.reviewBackground === ReviewBackgrounds.BLACK) return 'bg-slate-950';
  if (state.settings.reviewBackground === ReviewBackgrounds.LINE) return 'bg-[#06c755]';
  return 'bg-[linear-gradient(45deg,#e2e8f0_25%,transparent_25%),linear-gradient(-45deg,#e2e8f0_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#e2e8f0_75%),linear-gradient(-45deg,transparent_75%,#e2e8f0_75%)] bg-[length:14px_14px]';
}
...
async function downloadSelectedPng() {
  ...
  const exportResult = createPngExport({ sourceImage: state.source, frame, renderOptions: getRenderOptions(), fileName: `${frame.name || 'stixio-frame'}.png` });
  downloadDataUrl(canvasToPngDataUrl(exportResult.canvas), exportResult.fileName);
}
async function downloadZip() {
  if (!state.frames.length) return alert('Please import artwork first.');
  try {
    const exportResult = await createZipExport({ sourceImage: state.source, frames: state.frames, renderOptions: getRenderOptions(), renderedMap: state.rendered, exportOptions: { lineNamingMode: state.settings.lineNamingMode, prefix: 'stixio', allowWarningsOnly: true }, JSZipClass: window.JSZip });
    state.rendered = exportResult.renderedMap;
    state.reviewReport = exportResult.review;
