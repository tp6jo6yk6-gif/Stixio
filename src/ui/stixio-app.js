  ...
  return `
    <section class="relative overflow-hidden rounded-[2rem] bg-slate-950 text-white p-6 md:p-8 shadow-xl">
      ...
      <div class="relative grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-8 items-center">
        <div>
          <div class="mb-4 inline-flex rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs font-black text-emerald-200">Frame Editor v1</div>
          <h2 class="text-4xl md:text-6xl font-black leading-tight tracking-tight">Adjust faster.<br><span class="text-emerald-300">Export safer.</span></h2>
          <p class="mt-4 max-w-2xl text-sm md:text-base text-slate-300">Click a Frame, drag to move, pull the bottom-right handle to resize, use arrow keys for nudging, and snap to canvas or nearby Frames.</p>
        ...
        <div class="rounded-[1.5rem] border border-white/10 bg-white/10 p-4 backdrop-blur"><div class="grid grid-cols-2 gap-3">${metricCard('Frames', state.frames.length || '0', 'detected areas')}${metricCard('Selected', getSelectedFrame()?.name || 'None', 'active frame')}${metricCard('Rendered', state.rendered.size || '0', 'cached previews')}${metricCard('Review', getReviewSummaryLabel(), 'export readiness')}</div></div>
      </div>
    </section>
  `;
}
...
function bindFrameEditorEvents(root) {
  const canvas = root.querySelector('#sourceCanvas');
...
function handleCanvasPointerDown(event) {
  ...
  const hit = hitTestFrame(point);
  if (!hit) return;
  event.preventDefault();
  state.settings.selectedId = hit.frame.id;
  state.editorDrag = {
    type: hit.handle ? 'resize' : 'move',
    frameId: hit.frame.id,
    startPoint: point,
    startGeometry: { ...hit.frame.geometry }
  };
...
function handleCanvasPointerMove(event) {
  ...
  const point = getCanvasPoint(event);
  const hit = hitTestFrame(point);
  canvas.style.cursor = hit?.handle ? 'nwse-resize' : hit ? 'move' : 'crosshair';
  ...
  let nextFrame;
  if (drag.type === 'resize') {
    nextFrame = resizeFrame(frame, {
      ...drag.startGeometry,
      width: Math.max(MIN_FRAME_SIZE, drag.startGeometry.width + dx),
      height: Math.max(MIN_FRAME_SIZE, drag.startGeometry.height + dy)
    });
  } else {
    nextFrame = moveFrame({ ...frame, geometry: drag.startGeometry }, dx, dy);
  }
  nextFrame = clampFrameToSource(nextFrame);
  nextFrame = snapFrameToEditor(nextFrame);
  updateFrameInState(nextFrame);
  clearRenderCache();
  drawSourcePreview();
}
...
function handleKeydown(event) {
  ...
  const moved = snapFrameToEditor(clampFrameToSource(moveFrame(frame, x * step, y * step)));
  updateFrameInState(moved);
  clearRenderCache();
  renderSelected();
  runReview();
  refresh();
...
function getCanvasPoint(event) {
  const canvas = event.currentTarget || document.getElementById('sourceCanvas');
...
function deleteSelectedFrame() { const frame = getSelectedFrame(); if (!frame) return; state.frames = state.frames.filter(item => item.id !== frame.id); state.settings.selectedId = state.frames[0]?.id || null; clearRenderCache(); renderAll(); runReview(); refresh(); }
function duplicateSelectedFrame() { const frame = getSelectedFrame(); if (!frame) return; const copy = { ...frame, id: createId('frame'), name: `${frame.name} Copy`, geometry: { ...frame.geometry, x: frame.geometry.x + 24, y: frame.geometry.y + 24 }, state: { ...frame.state, selected: true } }; state.frames = [...state.frames.map(item => ({ ...item, state: { ...item.state, selected: false } })), clampFrameToSource(copy)]; state.settings.selectedId = copy.id; clearRenderCache(); renderAll(); runReview(); refresh(); }
